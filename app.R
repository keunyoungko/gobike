...
ggmap::register_google(key = "AIzaSyDGVQVEKBnzWwLHqTYjYjXgFQfJcBw0YfM")
...
# write_csv(age_duration_gender.data, "age_duration_gender_data.csv")
combined.data3 <- read_csv("final_combined_data.csv")
age_duration_gender.data <- read_csv("age_duration_gender_data.csv")
  ...
  mainPanel(
    tabsetPanel(
      tabPanel("The Demographic Trend of GoBikers",
               textOutput(outputId = "subscribers"),
               plotOutput(outputId = "monthly.trend")),
...
server <- function(input, output, session) {
  ...
  output$monthly.trend <- renderPlot({
  ...
  output$subscribers <- renderText({
    paste("The proportion of subscribers refers to how much of the total GoBikers are subscribed to bike sharing system (e.g. annual pricing).")
  })
